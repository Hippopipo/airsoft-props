...
After the game the lettered keys show statistics: `B` time each team held the point, `C` how many times each team captured it, `D` each team's longest unbroken hold, and `A` goes back to the scoreboard.
...
Press `*` to open the PIN-gated settings menu (default PIN `1234`) to change the game length, how often points are awarded, or the goal score (`0` turns the goal off so only the clock decides), or to start a new game. Each setting screen shows the current value until you start typing, and an out-of-range value beeps so you can try again. The game clock keeps running while the menu is open, and a new game length applies from the next game. The team buttons don't work while the menu is open, so it goes back to the scoreboard by itself after 15 seconds without a key press.
